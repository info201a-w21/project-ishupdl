...
pie = ggplot(uw_response, aes(x="", y=prop, fill=answer)) + geom_bar(stat="identity", width=1)

pie = pie + coord_polar("y", start=0) + geom_text(aes(label = paste0(round(prop), "%")), position = position_stack(vjust = 0.5))


pie = pie + scale_fill_manual(values=c("#55DDE0", "#33658A", "#2F4858", "#F6AE2D", "#F26419", "#999999")) 


pie = pie + labs(x = NULL, y = "Percentage of Votes per App", fill = NULL, title = "University of Washington Social Media Usage")
